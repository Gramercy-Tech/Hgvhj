...
let bpApp = {
  ...
  createRoot(){
    ...
    this.textureLoader.load("bp/home.png", (texture) => {
  ...
  showRoot(coordinates, immediate, showMolecules){
    ...
    if(immediate){
      ...
      if(showMolecules){
        let phases = bpApp.phases.map( (p) => p.children[0] );
        phases.forEach( (p,i) => {
          if(!p.parent.isSprouted){
            ...
            let tween = bpApp.showPhaseChildren(p, true) 
  ...
  createMoleculeNode( c ) {
    ...
    var shaderMaterial = new THREE.ShaderMaterial({
      uniforms: uniforms,
      vertexShader: Shaders.bp.vertexShader,
      fragmentShader: Shaders.bp.fragmentShader,
    })
    ...
    let labelMap = new THREE.TextureLoader().load("bp/" + c.label);
  ...
  createPhases(){
    //let self = this;
    let sprite1 = this.createPhaseNode("bp/Phase1.png", 'phase1');
    this.phases.push(sprite1);
    let sprite2 = this.createPhaseNode("bp/Phase2.png", 'phase2');
    this.phases.push(sprite2);
    let sprite3 = this.createPhaseNode("bp/Phase3.png", 'phase3');
    ...
    if(this.touchText){
      this.touchText.children.filter( c => c.isIcon ).forEach( (t) => {
        ...
        if(t.timeOffset < this.touchTextFade){
          ...
          t.material.opacity = t.timeOffset / bpApp.touchTextFade;
  ...
  createAllMolecules(){
    this.phases.map( (p) => p.children[0] ).forEach( (phaseObject) => {
      ...
      if(parentObject.sproutChildren === undefined){
        let childrenData = phaseChildrenBP[phaseObject.name];
        if(childrenData === undefined) debugger
  ...
  animateMolecules(deltaMS){
    ...
    if(bpApp.currentMode == "touchText" && this.rootChildren && this.rootChildren.children){
  ...
  highlightFilteredMolecules(){
    ...
    if(diseaseButton.length > 0) bpApp.highlightButton(diseaseButton[0]);
    if(sizeButton.length > 0) apApp.highlightButton(sizeButton[0]);
  },
  unfilterMolecules(){
  ...
  checkIfTexturesLoaded(){
    let num = Object.keys( phaseChildrenBP ).reduce( (a, b) => {
      return a + Object.keys(phaseChildrenBP[b]).length;
    }, 0);
    if(num == Object.keys(this.textures).length && $("#background-video")){
      Session.set("showSpinner", false);
      bpApp.runTouchTextMode();
      bpApp.showRoot([window.innerWidth/2, window.innerHeight/2]);
      bpApp.showPipelineText();
...
Template.bp.events({
  'click canvas': (event, template) => {
    bpApp.setIdleTimeout();
    if(bpApp.controlsEnabled){
      bpApp.handleClick(event);
    }
  },
  'click #see-all-button': (event, template) => {
    bpApp.hidePopup();
    bpApp.setIdleTimeout();
    bpApp.moveCameraTo(new THREE.Vector3(0,0,bpApp.cameraDistance));
    ...
    phases.forEach( (p) => {
      if(!p.parent.isSprouted){
        bpApp.showPhaseChildren(p) 
      }
    });
    bpApp.unfilterMolecules();
  },
  ...
  'click .area-under-investigation img': (event, template) => {
    ...
    if(bpApp.popUpActive){
      bpApp.hidePopup();
    }
    if(!selected){
      bpApp.addMoleculeFilter(filterBy, filterType);
    }else{

      bpApp.removeMoleculeFilter(filterBy, filterType);
    }
  },
});
...
Template.bp.rendered = () => {
  ...
  Object.keys(phaseChildrenBP).forEach( (p) => {
    phaseChildrenBP[p].forEach( (c) => {
      let assetPath = c.assetDirectory + "/" + "output.png";
      bpApp.textures[c.name] = bpApp.textureLoader.load(assetPath);
    });
  });
  ...
  bpApp.camera = camera;
  bpApp.canvas = renderer.domElement;
  bpApp.renderer = renderer;
  if(bpApp.focalLength) camera.setFocalLength(bpApp.focalLength);
  /*
  ...
  */
  ...
  camera.position.set( 0, 0, bpApp.cameraDistance );
  camera.lookAt( scene.position );
  ...
  function onResize(){
    ...
    if(window.orientation == 0 || window.orientation == 180){
      bpApp.rootGroupY = 250;
      bpApp.camera.position.z = bpApp.cameraDistance;
    }else{
      bpApp.camera.position.z = bpApp.cameraDistance - 60;
      bpApp.rootGroupY = 0;
    }
    bpApp.rootGroup.position.y = bpApp.rootGroupY
  }
  ...
  function update(){
    TWEEN.update();
    let delta = Date.now() - bpApp.time; 
    let clockDelta = clock.getDelta();
    if(bpApp.currentMode == "touchText"){
      bpApp.updateCanvasTouchText(clockDelta);
    }
    
    //bpApp.animateMolecules(clockDelta * 1000);
    let r = camera.getWorldRotation();
    bpApp.phases.forEach( (p) => {
      p.children[2].rotation.set( r.x, r.y, r.z, r.order );
      if(p.isSprouted){
        p.sproutChildren.rotation.set(r.x, r.y, r.z, r.order);
      }
    });
    if(bpApp.rootGroup && bpApp.rootGroup.children.length > 0){
      let c = bpApp.rootGroup.children[0];
      c.rotation.set( r.x, r.y, r.z, r.order );
    }
    let t = 800;
    bpApp.rootGroup.position.z += 0.1 * Math.sin(delta / t);
    bpApp.rootGroup.position.x += 0.15 * Math.sin(delta / t);
    //bpApp.rootGroup.position.y += 0.06 * Math.cos(delta / 500);
    bpApp.rootGroup.position.y += 0.15 * Math.cos(delta / t);
  }
}
