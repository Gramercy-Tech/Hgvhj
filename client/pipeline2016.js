...
let atsApp = {
  ...
  createAllMolecules(){
    this.phases.map( (p) => p.children[0] ).forEach( (phaseObject) => {
      ...
      if(parentObject.sproutChildren === undefined){
        let childrenData = phaseChildren2016[phaseObject.name];
  ...
  checkIfTexturesLoaded(){
    let num = Object.keys( phaseChildren2016 ).reduce( (a, b) => {
      return a + Object.keys(phaseChildren2016[b]).length;
    }, 0) - 1;
...
Template.pipeline2016.rendered = () => {
  ...
  Object.keys(phaseChildren2016).forEach( (p) => {
    phaseChildren2016[p].forEach( (c) => {
      let assetPath = c.assetDirectory + "/" + "output.jpg";
