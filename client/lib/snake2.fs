...
{
    float period = 6.28 * clamp(rev, 1., 4.);
    for (float a = 0.15; a < 100.; a += .015)
    {
        ...
        float c = cos(a);
        float s = sin(a);
        float c4 = cos(4. * mix(a, sin(a - u_time), abs(sin(u_time / 10.))));
        float p;
        if (PRESET == 0)
          p = radius * (exp(c) - 2. * c4 - pow(sin(a / 12.), 5.));
        else
          p = dot(vec2(.2,.9), vec2(c, s)) + dot(vec2(c4,.1), vec2(cos(a * radius), sin(a * radius)));
        vec2 um = vec2(c + sin(u_time)* .15, sin(a) * cos(u_time) * 1.5);
        #ifdef SNAKE
        if (a - .15 > 5.6 && a - .15 < 6.)
            um += (.1*(.4 - (6. - (a - .15))) / .4) * sin(a + u_time * 100.);
        #endif
        vec2 pos = uv - p * um;
        pos *= clamp(abs(sin(u_time / 25. - 15.)) * 2., .3, 1.5);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  uv.x *= iResolution.x / iResolution.y;
    ...
    if (PRESET == 0)
      ...
      b = f(uv, 2., abs(sin(u_time / 10.) * 0.4 + 0.2) * .8, 1.8);
    if (b.z > 0.)
    {
        b.w -= .15;
        #ifdef SNAKE
        if (b.w < 5.6 && b.w > .2)
            {
                c = texture2D(iChannel0, b.xz, 0.).xyz - vec3(0, 0, .2);
                c = mix(c, (texture2D(iChannel0, b.xy, 0.).x + .5) * vec3(.8), .75 * smoothstep(.5, 1., sin(texture2D(iChannel0, b.xy, 0.).x + b.w  * 40. + 13. * b.z)));
            }
        else if (b.w > .02 && b.w < 5.6)
          c = texture2D(iChannel0, (b.xw) * 2., 0.).xyz * vec3(1.2, 1.1, .1);
            else
                c = texture2D(iChannel1, b.xz * 2., 0.).xyz + vec3(0, 0, -.2);
