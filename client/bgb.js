...
let bgbApp = {
  ...
  cameraDistance: +FlowRouter.getQueryParam("d") || 1120,
  ...
  handleRootClick(event){
    ...
    this.mouse.x = coordinates[0] / $(this.canvas).innerWidth() * 2;
    this.mouse.y = coordinates[1] / $(this.canvas).innerHeight() * 2;
  ...
  handleMoleculeClick(event){
    let coordinates = this.mouseToScene([event.clientX, event.clientY]);
    this.mouse.x = coordinates[0] / $(bgbApp.canvas).innerWidth() * 2;
    this.mouse.y = coordinates[1] / $(bgbApp.canvas).innerHeight() * 2;
    this.raycaster.setFromCamera(this.mouse, camera);
  ...
  runRootMode(event){
    ...
    this.currentMode = "root";
    //this.showRoot([$(bgbApp.canvas).innerWidth()/2, $(bgbApp.canvas).innerHeight()/2]);
    this.showSeeAll();
    this.hideFilterMenu();
    ...
    tween.onComplete( () => {
      //this.setBoomerangOpacity( 1.0 );
      this.showRoot([$(bgbApp.canvas).innerWidth() / 2, $(bgbApp.canvas).innerHeight() / 2], true, true);
      //this.showPhases();
    });
    tween.start();
  },
  createTouchTextRootAnimationCycle(){
    ...
    this.touchTextRootCycle = Meteor.setInterval( () => {
      ...
      let tween = this.createFadeOutRootTween();
      tween.onComplete( () => {
        this.showRoot([$(bgbApp.canvas).innerWidth()/2, $(bgbApp.canvas).innerHeight()/2], true);
      });
      tween.start();
    }, 20 * 1000);
    */
  ...
  createRoot(){
    ...
    this.textureLoader.load("moleculeImages/circle_home.png", (texture) => {
      ...
      //bgbApp.showRoot([0,0], true);
      bgbApp.createPhases();
      bgbApp.createAllMolecules();
      bgbApp.addReferencesText();
    });
  },
  showRoot(coordinates, immediate, showMolecules){
    ...
    if(immediate){
      ...
      if(showMolecules){
        let phases = bgbApp.phases.map( (p) => p.children[0] );
        phases.forEach( (p,i) => {
          if(!p.parent.isSprouted){
            ...
            let tween = bgbApp.showPhaseChildren(p, true) 
  ...
  createMoleculeNode( c ) {
     ...
     */
    var uniforms = {
      u_width: { type: "f", value: $(bgbApp.canvas).innerWidth() },
      u_height: { type: "f", value: $(bgbApp.canvas).innerHeight() },
  ...
  createCanvasTouchText(){
    ...
    //Create text sprite
    this.textureLoader.load("moleculeImages/touch_transparent_2_resized.png", (texture) => {
    ...
    if(this.touchText){
      this.touchText.children.filter( c => c.isIcon ).forEach( (t) => {
        ...
        if(t.timeOffset < this.touchTextFade){
          ...
          t.material.opacity = t.timeOffset / bgbApp.touchTextFade;
  ...
  mouseToScene(coordinates){
    let [x,y] = coordinates;
    //const sceneX = (( x / $(bgbApp.canvas).innerWidth() ) * 2 - 1) * $(bgbApp.canvas).innerWidth();
    //const sceneY = (-1 * ( y / $(bgbApp.canvas).innerHeight() ) * 2 + 1) * $(bgbApp.canvas).innerHeight();
    const sceneX = (( x / $(bgbApp.canvas).innerWidth() ) * 2 - 1) * $(bgbApp.canvas).innerWidth() / 2;
    const sceneY = (-1 * ( y / $(bgbApp.canvas).innerHeight() ) * 2 + 1) * $(bgbApp.canvas).innerHeight() / 2;
  ...
  animateMolecules(deltaMS){
    ...
    if(bgbApp.currentMode == "touchText" && this.rootChildren && this.rootChildren.children){
  ...
  highlightFilteredMolecules(){
    ...
    if(diseaseButton.length > 0) bgbApp.highlightButton(diseaseButton[0]);
    if(sizeButton.length > 0) bgbApp.highlightButton(sizeButton[0]);
  },
  unfilterMolecules(){
  ...
  checkIfTexturesLoaded(){
    ...
    if(num == Object.keys(this.textures).length && $("#background-video")){
      ...
      //this.canvas.width = $(bgbApp.canvas).innerWidth();
      //this.canvas.height = $(bgbApp.canvas).innerHeight();

      this.renderer.setSize($(bgbApp.canvas).innerWidth(), $(bgbApp.canvas).innerHeight());
      this.camera.aspect = $(bgbApp.canvas).innerWidth() / $(bgbApp.canvas).innerHeight();
...
Template.bgb.events({
  'click canvas': (event, template) => {
    bgbApp.setIdleTimeout();
    if(bgbApp.controlsEnabled){
      bgbApp.handleClick(event);
    }
  },
  'click #see-all-button': (event, template) => {
    bgbApp.hidePopup();
    bgbApp.setIdleTimeout();
    bgbApp.moveCameraTo(new THREE.Vector3(0,0,bgbApp.cameraDistance));
    ...
    phases.forEach( (p) => {
      if(!p.parent.isSprouted){
        bgbApp.showPhaseChildren(p) 
      }
    });
    bgbApp.unfilterMolecules();
  },
  'click #references-button': (event, template) => {
    if(!bgbApp.popUpActive){
      bgbApp.showReferences();
    }else{
      bgbApp.hidePopup();
    }
    bgbApp.setIdleTimeout();
  ...
  'click .area-under-investigation img': (event, template) => {
    ...
    if(!selected){
      bgbApp.addMoleculeFilter(filterBy, filterType);
    }else{

      bgbApp.removeMoleculeFilter(filterBy, filterType);
    }
  },
  /* Events added for bgb menu */
  'click .button': (event, template) => {
	let buttonID = $(event.target).attr('id').replace('-btn','');
    if(buttonID == "trials"){
	  $('#background-video').fadeOut(500);
    }else{
	  $('#background-video').fadeIn(500);
    }
	if ($('#buttons').hasClass('home')) {
	  $(event.target).addClass('selected');
	  $('#buttons p').fadeOut(100);
	  $('#buttons').removeClass('home').addClass('header');
	  ...
	  if ($(event.target).attr('id') == 'home-btn') {
...
Template.bgb.rendered = () => {
  ...
  Object.keys(phaseChildren).forEach( (p) => {
    phaseChildren[p].forEach( (c) => {
      let assetPath = c.assetDirectory + "/" + "output.jpg";
      bgbApp.textures[c.name] = bgbApp.textureLoader.load(assetPath);
    });
  });
  ...
  camera = new THREE.PerspectiveCamera( 35, $(canvas).innerWidth() / $(canvas).innerHeight(), 1, 10000 );
  bgbApp.camera = camera;
  bgbApp.canvas = renderer.domElement;
  bgbApp.renderer = renderer;
  if(bgbApp.focalLength) camera.setFocalLength(bgbApp.focalLength);
  /*
  ...
  */
  ...
  camera.position.set( 0, 0, bgbApp.cameraDistance );
  camera.lookAt( scene.position );
  ...
  */
  ...
  function onResize(){
    ...
    if(window.orientation == 0 || window.orientation == 180){
      ...
      bgbApp.camera.position.z = bgbApp.cameraDistance;
    }else{
      bgbApp.camera.position.z = bgbApp.cameraDistance - 60;
      bgbApp.rootGroupY = 0;
    }
    bgbApp.rootGroup.position.y = bgbApp.rootGroupY
    */
  }
  ...
  function update(){
    TWEEN.update();
    let delta = Date.now() - bgbApp.time; 
    let clockDelta = clock.getDelta();
    if(bgbApp.currentMode == "touchText"){
      bgbApp.updateCanvasTouchText(clockDelta);
    }
    
    bgbApp.animateMolecules(clockDelta * 1000);
    let r = camera.getWorldRotation();
    bgbApp.phases.forEach( (p) => {
      p.children[2].rotation.set( r.x, r.y, r.z, r.order );
      if(p.isSprouted){
        p.sproutChildren.rotation.set(r.x, r.y, r.z, r.order);
      }
    });
    if(bgbApp.rootGroup && bgbApp.rootGroup.children.length > 0){
      let c = bgbApp.rootGroup.children[0];
      c.rotation.set( r.x, r.y, r.z, r.order );
    }
    let t = 800;
    bgbApp.rootGroup.position.z += 0.1 * Math.sin(delta / t);
    bgbApp.rootGroup.position.x += 0.15 * Math.sin(delta / t);
    //bgbApp.rootGroup.position.y += 0.06 * Math.cos(delta / 500);
    bgbApp.rootGroup.position.y += 0.15 * Math.cos(delta / t);
  }
}
